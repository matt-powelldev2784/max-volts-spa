...
const getInvoices = async (sortBy: SortField, sortOrder: SortOrder, page: number, searchTerm: string) => {
    ...
    <section className="w-11/12 mx-auto flexCol">
      ...
      <div className="flex flex-col md:flex-row w-full justify-between items-start md:items-end px-0 md:px-2 mb-4 gap-2 md:gap-8 mt-4 md:mt-4">
        <div className="flexRow gap-4 mb-4 md:mb-0">
          <h1 className="text-2xl text-gray-800">Invoice List</h1>
          <Button
      ...
      <Table className="table md:hidden table-fixed">
        ...
        <TableBody>
          {invoices?.map((invoice) => (
            <TableRow key={invoice.id} data-testid="invoice-product-row" className="hover:bg-muted transition">
              <TableCell>
                <DropDownMenu invoiceId={invoice.id} clientId={invoice.client_id} />
              </TableCell>

              <TableCell className="truncate">{invoice.id}</TableCell>
              <TableCell className="truncate">{invoice.client?.name}</TableCell>
              <TableCell className="pr-4 truncate">£{invoice.total_value?.toFixed(2) ?? '0.00'}</TableCell>
            </TableRow>
          ))}
        ...
        <TableBody>
          {invoices?.map((invoice) => (
            <TableRow key={invoice.id} data-testid="invoice-product-row" className="hover:bg-muted transition">
              ...
              <TableCell className="truncate" title={invoice.client?.name || ''}>
                {invoice.client?.name}
              </TableCell>

              <TableCell className="truncate" title={invoice.client?.company || ''}>
...
const DropDownMenu = ({ invoiceId, clientId }: DropDownMenuProps) => {
